    ...
    'card_buttons': async (cardButtonsContext) => {
        const buttons = [{
            text: 'Card_Title',
              ...
              // The SDK auto-refreshes the access token when get/post/... get a 401.
              // Call refreshToken() yourself only when you need a fresh token up front —
              // e.g. before a long batch of requests, or to hand a token off to another client.
              const { access_token, expires_at } = await api.refreshToken();
              ...
              const cards = await api.get('/api/v1/cards?limit=50');
            callback: async (ctx) => {
